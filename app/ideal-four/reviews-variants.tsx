...
type MobileReviewPage =
  | { kind: "video"; video: Review }
  | { kind: "text"; reviews: Review[] }

function buildMobileReviewPages(reviews: Review[]): MobileReviewPage[] {
  const videos = reviews.filter((review) => review.type === "video" && review.videoUrl)
  const texts = reviews.filter((review) => review.type === "text")
  const textPages = Array.from({ length: Math.ceil(texts.length / 2) }, (_, index) =>
    texts.slice(index * 2, index * 2 + 2),
  )
  const pages: MobileReviewPage[] = []
  const groupCount = Math.max(videos.length, textPages.length)

  for (let index = 0; index < groupCount; index += 1) {
    if (videos[index]) pages.push({ kind: "video", video: videos[index] })
    if (textPages[index]?.length) pages.push({ kind: "text", reviews: textPages[index] })
  }

  return pages.length ? pages : [{ kind: "text", reviews: reviews.slice(0, 2) }]
}
...
export function ReviewsVariantHero({ apiUrl }: { apiUrl?: string }) {
  ...
  const mobilePages = buildMobileReviewPages(reviews)
  const safePage = Math.min(page, screens.length - 1)
  const safeMobilePage = Math.min(mobilePage, mobilePages.length - 1)
  const screen = screens[safePage]
  const mobileScreen = mobilePages[safeMobilePage]
  ...
  const goMobile = (nextPage: number) => {
    setMobilePage((nextPage + mobilePages.length) % mobilePages.length)
  }

  return (
    <div
      data-review-variant="mosaic-dark"
      className="relative left-1/2 w-screen -translate-x-1/2 overflow-hidden bg-transparent pt-4 pb-4 shadow-[0_35px_100px_-55px_rgba(0,0,0,0.95)] md:pt-6 md:pb-6"
    >
      <Image
        src="/logo-orient.png"
        alt=""
        ...
        width={620}
        height={620}
        className="pointer-events-none absolute -left-40 top-1/2 hidden w-[580px] -translate-y-1/2 rotate-[10deg] select-none opacity-[0.028] md:block"
      />
      <Image
        src="/logo-orient.png"
        alt=""
      ...
      />
      <div className="relative mx-auto w-full px-4 md:hidden">
        <div className="mb-3 flex items-center justify-center gap-3">
          <Stars rating={aggregate.rating} className="gap-0.5 [&_svg]:h-4 [&_svg]:w-4" />
        ...
        <motion.div
          key={`mobile-review-page-${safeMobilePage}`}
          initial={{ opacity: 0, y: 24 }}
        ...
        >
          {mobileScreen.kind === "video" ? (
            <div className="flex h-full w-full items-stretch justify-center">
              <VideoReviewCard
                key={`mobile-video-${safeMobilePage}-${mobileScreen.video.id}`}
                review={mobileScreen.video}
                onOpen={() => setOpen(mobileScreen.video)}
                mobile
              />
            </div>
          ) : (
            <div
              key={`mobile-text-${safeMobilePage}`}
              className={`grid h-full gap-3 ${
                mobileScreen.reviews.length > 1 ? "grid-rows-2" : "grid-rows-1"
              }`}
            >
              {mobileScreen.reviews.map((review, index) => (
                <TextReviewCard
                  key={review.id}
                  review={review}
                  delay={index * 0.06}
        ...
        {mobilePages.length > 1 && (
          <ScreenPager
            page={safeMobilePage}
            count={mobilePages.length}
            onPrev={() => goMobile(safeMobilePage - 1)}
            onNext={() => goMobile(safeMobilePage + 1)}
            onSelect={goMobile}
          />
        )}
...
function ReviewColumn({
  ...
  return (
    <div
      className={
        "order-2 grid min-w-0 gap-3 self-stretch sm:gap-4 lg:h-full lg:grid-rows-2 " +
...
function ReviewCardContent({
  ...
  return (
    <>
      ...
      <div className={`relative min-w-0 pr-20 ${mobile ? "mb-2" : "mb-4"}`}>
      ...
      {isLong && (
        <button
          type="button"
          onClick={onOpen}
          className={`${mobile ? "mt-2" : "mt-3"} inline-flex items-center gap-1 text-[13px] font-semibold text-[#c9a86e] transition-colors hover:text-[#e8c98a]`}
        >
...
function TextReviewCard({
  ...
  return (
    <motion.article
      ...
      className={
        "group relative flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border border-white/[0.075] bg-gradient-to-br from-[#1a2332] to-[#0e1720] p-4 text-white shadow-lux transition-colors hover:border-[#c9a86e]/40 " +
        (mobile ? "min-h-0" : "min-h-[240px] sm:min-h-[280px] sm:p-5 lg:min-h-0")
      }
    >
      <ReviewCardContent review={review} onOpen={onOpen} mobile={mobile} />
    </motion.article>
  )
}
...
function VideoReviewCard({
  ...
  return (
    <motion.button
      ...
      className={
        "group relative order-1 mx-auto aspect-[9/16] overflow-hidden rounded-2xl border border-[#c9a86e]/25 bg-[#080c12] text-left shadow-[0_30px_75px_-30px_rgba(0,0,0,0.95)] transition-[border-color,box-shadow] duration-300 hover:border-[#c9a86e]/65 hover:shadow-[0_34px_85px_-30px_rgba(201,168,110,0.32)] " +
        (mobile
          ? "block h-full w-auto max-w-full"
          : "w-full max-w-[280px] sm:max-w-[330px] md:col-span-2 md:max-w-[350px] lg:order-2 lg:col-span-1 lg:max-w-none")
      }
    >
      <Image
        src={review.poster!}
...
function ScreenPager({
  ...
  return (
    <div className="relative mt-8 flex items-center justify-center gap-3 sm:mt-9 sm:gap-4 md:mt-10 md:gap-6">
      <button
        type="button"
        onClick={onPrev}
        aria-label="Предыдущий экран отзывов"
        title="Предыдущий экран"
        className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#c9a86e]/35 bg-[#111927] text-white transition-all hover:bg-[#c9a86e] hover:text-[#0e1720] active:scale-95 sm:h-11 sm:w-11"
      >
